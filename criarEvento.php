...
if (isset($_POST['inserir'])) {
  ...
  $dataAtual->setTime(0, 0, 0);  
    ...
    try {
        $dataEventoObj = new DateTime($dataDoEvento); // Tenta criar um objeto DateTime a partir da string
        $dataEventoObj->setTime(0, 0, 0); // Zera a hora para comparar apenas o dia
    } catch (Exception $e) {
        // Lidar com o caso de a data ser inválida (ex: formato incorreto)
        $_SESSION['mensagem_erro'] = "Formato de data inválido.";
        header("Location: criarEvento.php");
        exit;
    }
    
    // Se a data do evento for anterior à data atual
    if ($dataEventoObj < $dataAtual) {
        $_SESSION['mensagem_erro'] = "A data do evento não pode ser anterior ao dia de hoje.";
        header("Location: criarEvento.php"); // Redireciona de volta para o formulário
...
<body class="bg-ligth text-dark">
  ...
  <main class="container my-5  bg-ligth text-dark rounded p-4 shadow">
    ...
    <form autocomplete="off" action="" method="post" id="form-endereco" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        <label class="form-label" for="dataEvento">Dia do Evento:</label>
        <input class="form-control" type="date" id="dataEvento" name="datas" required />
        <div class="invalid-feedback" id="feedbackDataEvento">
        </div>
      </div>
  ...
  <script src="js/validacao-evento.js"></script>
</body>
