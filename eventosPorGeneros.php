      ...
      <article class="col-12">
        <?php if (count($dados) > 0) { ?>
          <h2 class="text-center">
            eventos sobre <span class="badge bg-primary">
              <?= $dados[0]["genero"] ?>
            </span>
          </h2>
        <?php } else { ?>
    ...
    <?php foreach ($dados as $tipoEvento) { ?> 
        <div class="col-md-4"> 
            <article class="card **card-com-imagem** bg-secondary text-light h-100" 
                     style="background-image: url('images/<?= htmlspecialchars($tipoEvento['imagem']) ?>'); background-repeat: no-repeat; background-size: cover; background-position: center; min-height: 300px; display: flex; flex-direction: column;">
                <a href="evento.php?id=<?= $tipoEvento["id"] ?>" class="list-group-item list-group-item-action card-link">
                    <div class="card-body fundo-opaco">
                        <h5 class="card-title">Evento: <?= htmlspecialchars($tipoEvento['evento']) ?></h5>
                        <p class="card-text">Data: <?= htmlspecialchars(Utils::formatarDataBr($tipoEvento['data_evento'])) ?></p>
                        <p class="card-text">Horário: <?= htmlspecialchars($tipoEvento['horario']) ?></p>                           
                </a>
            </article>
        </div>
    <?php } ?>
